...
> -   **External VNC based on TightVNC**: like the PuTTY based plugin, the external TightVNC connection is established by passing on the password through the command-line. Auto logon for external VNC connections is therefore not supported for locked down documents which do not allow to reveal passwords.
