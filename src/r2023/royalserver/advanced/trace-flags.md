...
| TRACEFLAG_LOAD_USER_PROFILE_FOR_SCRIPTS           | Run dynamic folder/credential scripts with the requesting user's real Windows profile loaded instead of a transient, per-run scratch profile (5.04.50925+)          |
| TRACEFLAG_USE_BATCH_LOGON_FOR_SCRIPTS             | Run dynamic folder/credential scripts using a batch logon (full token) instead of an interactive logon (UAC-filtered); affects administrators only (5.04.50925+)    |
